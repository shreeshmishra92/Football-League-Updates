...
export class HeaderComponent implements OnInit {
  ...
  getLeague(country: countries) {
    ...
    let leagueName = TopLeagues[country.name as keyof typeof TopLeagues];
      ...
      this.footballDataService.getLeaguesId(country.code, this.currentSeason, leagueName, country.name).subscribe((res: any) => {
      ...
      getStandings(leagueId:number){               
          this.footballDataService.getStandings(leagueId, this.currentSeason).subscribe((data:any)=>{
            if(data['response'].length>0){ this.standings=data['response'][0]?.league.standings[0];
              window.localStorage.setItem('standings', JSON.stringify(data['response'][0].league.standings[0]));
             }
