...
export class FootballappService {
  ...
  getLeaguesId(countryCode: string, season: number, leagueName: string, countryName: string): Observable<any> {
  ...
  showActiveClass(): Observable<any> {
    if (localStorage.getItem('selectedCountry')) {
      var countryData = JSON.parse(localStorage.getItem('selectedCountry') || '{}');
    } else {
      countryData = {"name": "England","code": "GB", "flag": "https://media-4.api-sports.io/flags/gb.svg"};
    }
    return countryData;
  }
}
